Shader "Unlit/L11_OldSchoolPro"
{
...

    {
        ...
        [Header(Texture)]
        // [HideInInspect]
            [HideInInspector] _MainTex    ("RGB:基础颜色 A:环境遮罩", 2D)     = "white" {}
...

    {
...

        {
            ...
            float3 TriColAmbien(float3 nDirWS,float3 EnvUpCol,float3 EnvSideCol,float3 EnvDownCol){
                // 光照模型(环境光照部分)
                ...
                float3 envCol = EnvUpCol * upMask +
                                EnvSideCol * sideMask +
                                EnvDownCol * downMask;     // 混合环境色
                                //三颜色环境光
                return envCol;
            }
...

            {
                ...
                float3 envCol = TriColAmbien( nDirWS, _EnvUpCol, _EnvSideCol, _EnvDownCol);
